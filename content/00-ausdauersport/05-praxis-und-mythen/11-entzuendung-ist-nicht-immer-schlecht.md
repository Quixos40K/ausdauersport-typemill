...
```mermaid
flowchart TD
    A[Training setzt einen Reiz] --> B[Entzündungsreaktion]

    B --> C{Reaktion klingt ab?}

    C -->|Ja| D[Reparatur startet]
    D --> E[Gewebeumbau]
    E --> F[Anpassung wird möglich]

    C -->|Nein| G[Reizung bleibt bestehen]
    G --> H[Schmerz oder Schwellung]
    H --> I[Belastung prüfen]

    F --> J[Entzündung ist ein Signal]
    I --> J

    J --> K[Richtig einordnen statt pauschal unterdrücken]
