...
```mermaid
flowchart TD
    A[Mythos: Dehnen verhindert Verletzungen] --> B[Dehnen allein reicht nicht]

    B --> C[Belastung steigt zu schnell]
    B --> D[Kraft fehlt]
    B --> E[Erholung fehlt]

    C --> F[Gewebe wird überfordert]
    D --> F
    E --> F

    F --> G[Verletzungsrisiko kann steigen]
...
```mermaid
flowchart TD
    A[Dehnen als ergänzender Baustein] --> B[Beweglichkeit verbessern]
    A --> C[Spannungsgefühl reduzieren]
    A --> D[Körperwahrnehmung verbessern]

    B --> E[Gezielt und ruhig einsetzen]
    C --> E
    D --> E

    E --> F[Belastung sinnvoll steuern]
    E --> G[Kraft aufbauen]
    E --> H[Erholung beachten]

    F --> I[Bessere Belastbarkeit]
    G --> I
    H --> I
