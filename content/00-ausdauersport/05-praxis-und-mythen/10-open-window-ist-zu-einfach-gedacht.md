...
```mermaid
flowchart TD
    A[Harte Ausdauerbelastung] --> B[Open-Window-Bild]
    B --> C[Immunsystem wirkt offen]
    C --> D[Zu einfache Erklärung]

    A --> E[Bessere Einordnung]
    E --> F[Immunzellen verändern ihre Verteilung]
    F --> G[Belastung löst Stressreaktion aus]

    G --> H[Kontext entscheidet]
    H --> I[Energieverfügbarkeit]
    H --> J[Schlaf und Erholung]
    H --> K[Kontakt mit Erregern]

    I --> L[Regeneration wird beeinflusst]
    J --> L
    K --> M[Infektrisiko steigt durch Exposition]

    L --> N[Nicht nur Training entscheidet]
    M --> N

    N --> O[Open Window ist zu einfach gedacht]
    ...
    class E,F,G,H,I,J,K,L,M,N primary;
    class O dark;
```
