...
```mermaid
flowchart TD
    A[Supplemente im Ausdauersport] --> B[Ernährung ist die Basis]

    B --> C[Energiezufuhr sichern]
    C --> D[Kohlenhydrate und Protein einplanen]
    D --> E[Flüssigkeit und Mikronährstoffe beachten]

    E --> F{Konkreter Bedarf?}

    F -->|Nein| G[Kein Supplement nötig]
    F -->|Ja| H[Gezielt ergänzen]
    ...
    I --> M[Verträglichkeit prüfen]
    J --> M
    K --> M
    L --> M

    G --> N[Basis bleibt entscheidend]
    M --> N

    N --> O[Supplemente ergänzen Ernährung]
    O --> P[Sie ersetzen sie nicht]
