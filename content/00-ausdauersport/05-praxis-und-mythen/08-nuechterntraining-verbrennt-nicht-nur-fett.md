...
```mermaid
flowchart TD
    A[Nüchterntraining] --> B[Mythos: verbrennt nur Fett]

    B --> C[Fettanteil kann akut steigen]
    C --> D[Kohlenhydrate werden trotzdem genutzt]
    D --> E[Kein automatischer Fettverlust]

    A --> F[Einheit richtig einordnen]
    F --> G{Locker und kurz?}

    G -->|Ja| H[Kann gezielt passen]
    H --> I[Als Werkzeug nutzen]

    G -->|Nein| J[Intensiv oder lang]
    J --> K[Kohlenhydrate werden wichtiger]
    K --> L[Trainingsqualität kann leiden]

    E --> M[Gesamtkontext entscheidet]
    I --> M
    L --> M

    M --> N[Ernährung, Ziel und Erholung zählen]
    ...
    class A dark;
    class B,E,J,L highlight;
    class C,D,F,G,H,I,K,M primary;
    class N dark;
```
