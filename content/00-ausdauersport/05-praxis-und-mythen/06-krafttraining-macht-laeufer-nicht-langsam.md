...
```mermaid
flowchart TD
    A[Krafttraining für Läufer] --> B[Muskeln geben Halt]

    B --> C[Beinachse wird stabiler]
    B --> D[Rumpf und Hüfte kontrollieren besser]
    B --> E[Wade und Fuß arbeiten belastbarer]

    C --> F[Weniger Ausweichbewegungen]
    D --> F
    E --> G[Kräfte werden besser aufgenommen]

    F --> H[Bessere Kraftübertragung]
    G --> H

    H --> I[Abdruck wird effizienter]
    F --> J[Spitzenlasten sinken]
    G --> J

    I --> K[Laufökonomie kann profitieren]
    J --> L[Verletzungsrisiko kann sinken]

    K --> M[Krafttraining macht nicht langsam]
    L --> M

    A --> N[Falsch geplant]
    N --> O[Zu viel oder ungünstig platziert]
    O --> P[Muskelkater vor wichtigen Läufen]
    P --> Q[Laufqualität leidet]

    Q --> R[Dosis und Timing entscheiden]
    M --> R
    ...
    class A dark;
    class B,C,D,E,F,G,H,I,J,K,L primary;
    class N,O,P,Q highlight;
    class M,R dark;
```
