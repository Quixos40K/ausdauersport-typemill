...
```mermaid
flowchart TD
    A[HIIT im Ausdauertraining] --> B{Wie wird HIIT eingesetzt?}

    B -->|Als Ersatz| C[Grundlagentraining fehlt]
    C --> D[Umfang bleibt zu gering]
    D --> E[Ermüdung steigt schnell]
    E --> F[Outcome: instabile Ausdauerbasis]

    B -->|Als Ergänzung| G[Grundlage bleibt erhalten]
    G --> H[Intensive Reize werden dosiert]
    H --> I[Tempo und VO2max werden gezielt trainiert]
    I --> J[Outcome: bessere Leistungsentwicklung]
