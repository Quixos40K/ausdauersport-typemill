...
```mermaid
flowchart TD
    A[Schweiß beim Training] --> B[Temperaturregulation]
    A --> C[Gefühl: Training war hart]

    B --> D[Schweißmenge steigt]
    D --> E[Umwelt und Kleidung]
    D --> F[Person und Hitzegewöhnung]

    E --> G[Mehr Schweiß möglich]
    F --> G

    C --> H[Trugschluss möglich]
    G --> H

    H --> I[Kein direktes Qualitätsmaß]

    I --> J[Trainingsqualität anders prüfen]
    J --> K[Intensität passend]
    J --> L[Erholung und Technik stabil]
    J --> M[Ziel der Einheit erfüllt]

    K --> N[Sinnvolle Einordnung]
    L --> N
    M --> N
    ...
    class A dark;
    class B,D,E,F,G primary;
    class C,H,I highlight;
