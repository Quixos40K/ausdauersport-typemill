...
```mermaid
flowchart TD
    A[Laktat bei Belastung] --> B[Entsteht bei der Energiebereitstellung]

    B --> C{Ist Laktat ein Abfallprodukt?}

    C -->|Mythos| D[Falsche Einordnung]
    D --> E[Laktat wird nur mit Ermüdung verbunden]

    C -->|Nein| F[Zwischenprodukt des Stoffwechsels]

    E --> F

    F --> G[Kann als Energie genutzt werden]
    G --> H[Wird zwischen Geweben transportiert]
    H --> I[Laktat-Shuttle]

    I --> J[Hilft, Intensität und Schwellen besser einzuordnen]
