# Energieverfügbarkeit und Makronährstoffe
...
```mermaid
graph TD
    A(Energieverfuegbarkeit) --> B(Energiezufuhr)
    A --> C(Trainingsverbrauch)
    B --> D(Makronährstoffe)
    C --> E(Restenergie fuer Körper)
    D --> F(Kohlenhydrate)
    D --> G(Protein)
    D --> H(Fette)
    F --> I(Intensive Belastung)
    G --> J(Reparatur und Anpassung)
    H --> K(Hormone und Grundversorgung)
    E --> L(Regeneration und Leistungsentwicklung)
    E --> M(Niedrige Verfügbarkeit)
    M --> N(Müdigkeit und Anpassungsprobleme)
