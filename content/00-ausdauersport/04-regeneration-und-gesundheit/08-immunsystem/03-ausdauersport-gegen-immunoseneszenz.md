...
```mermaid
flowchart TD
    A[Regelmäßiger Ausdauersport] --> B[Immunzellen mobilisieren]
    A --> C[Entzündung regulieren]
    A --> D[Zellstoffwechsel verbessern]

    B --> E[Redistribution]
    C --> F[Resolution statt Dauerstress]
    D --> G[Metabolische Effizienz]

    E --> H[Bessere Immunüberwachung]
    F --> H
    G --> H

    H --> I[Stabilere Immunbalance]
    I --> J[Abhängig von Schlaf Energie und Dosierung]
...
```mermaid
flowchart TD
    A[Immunoseneszenz] --> B[Weniger Zellflexibilität]
    A --> C[Mehr Entzündungsneigung]

    B --> D[Langsamere Immunreaktion]
    C --> E[Chronisch niedrige Entzündung]

    D --> F[Weniger Anpassungsfähigkeit]
    E --> F

    F --> G[Training muss gut dosiert sein]
