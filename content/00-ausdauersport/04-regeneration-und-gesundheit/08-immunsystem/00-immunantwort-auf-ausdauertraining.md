...
```mermaid
flowchart TD
    A[Ausdauertraining] --> B[Akute Belastung]
    B --> C[Immunzellen im Blut steigen]
    B --> D[Stresshormone steigen]
    B --> E[Entzündungssignale steigen]

    C --> F[Umverteilung der Immunzellen]
    D --> F
    E --> G[Reparatur und Anpassung]

    F --> H[Regeneration entscheidet]
    G --> H

    H --> I[Ausreichend Schlaf und Energie]
    H --> J[Zu viel Stress und zu wenig Erholung]

    I --> K[Stabile Anpassung]
    J --> L[Höhere Anfälligkeit]
