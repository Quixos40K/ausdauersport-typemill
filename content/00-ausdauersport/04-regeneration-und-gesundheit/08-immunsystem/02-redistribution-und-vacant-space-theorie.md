...
```mermaid
flowchart TD
    A[Ausdauerbelastung] --> B[Stresssignale steigen]
    B --> C[Immunzellen ins Blut]
    C --> D[Akute Mobilisierung]

    D --> E[Belastung endet]
    E --> F[Zellen verlassen Blut]
    F --> G[Redistribution ins Gewebe]

    G --> H[Immunüberwachung]
    G --> I[Reparatur und Regulation]

    H --> J[Normale Anpassung]
    I --> J

    D --> K[Wiederholte Mobilisierung]
    K --> L[Vacant Space Theorie]
    L --> M[Möglicher Raum für neue Zellbalance]

    J --> N[Abhängig von Erholung]
    M --> N
    N --> O[Schlaf Energie Belastungssteuerung]
